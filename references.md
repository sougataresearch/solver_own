...
| [`PyRCWA`](../PyRCWA) | Python | Added at the project owner's explicit request (github.com/vitamingcheng/PyRCWA, MIT license). Small (v0.0.1, 6 stars, 12 commits, no test suite) but the actual RCWA math is a real, coherent general 2D P/Q eigenoperator implementation (same architecture family as this project's own `eigenmodes.py`/`smatrix.py`, including an explicit "free space" gap-medium reference basis) — assessed and used *despite* the low community-adoption signal because the code itself checked out on direct reading, not on reputation. **Second, structurally-different 1D-grating oracle for Phase 3**, as of `tests/oracles/rcwa_1d_pyrcwa.py`: `pyrcwa/solver.py`/`core.py`/`matrix.py` hand-transcribed (not imported — same rule as every other oracle here, and practically necessary: the repo targets pre-2.0 NumPy, using `np.NAN`/`np.mat`, both removed in NumPy 2.0, so it cannot even be imported in this project's environment without a numpy-global monkeypatch). Scoped to **normal incidence only** — PyRCWA's `(alpha, theta)` oblique-angle convention wasn't confidently mapped onto this project's `(theta, phi)`, so the comparison is restricted to the one regime where they provably coincide, rather than asserting an unverified equivalence. **Verified against a live run of the actual repo before trusting the transcription** (a real bug-finding step, not skipped): a live run (via the numpy-monkeypatch, used only for this one-off check, never in the permanent oracle) at `fft_resolution=(2001,2001)`/`(4001,4001)`/`(8001,8001)` gave `Total R = 0.912281 / 0.912194 / 0.912151`, monotonically converging toward `sougata_solver`'s own `0.912109` — confirming the formulas agree and the residual gap is PyRCWA's own FFT/raster discretization error, not a real mismatch. The transcription itself uses this project's already-validated analytic Fourier-coefficient convolution matrix (`rcwa_1d_gaylord._toeplitz_convolution_matrix`, reused not reimplemented) in place of PyRCWA's FFT-of-raster route (same substitution `rcwa_1d_gaylord.py` already made, same reason) — matches `sougata_solver` to ~1e-10 (`tests/test_1d_grating.py::test_te_matches_pyrcwa_oracle_at_normal_incidence`), confirming both the transcription's fidelity and that it targets the same converged answer the live run approaches. A first draft of the transcription had one real bug, caught by testing before trusting it: used a per-diffraction-order `Kz` matrix where the source (`solver.py:168`) actually reads a single scalar (the incident wave's own `kz`, `self.K[2]/k0`) — `self.Kz` is computed by `compute_wave_vector` but never actually read anywhere in `compute_diffraction_efficiency`; the bug produced `nan` immediately (a loud failure, not a silently wrong number), fixed by re-reading the source line-by-line rather than assumed correct from the general pattern of the surrounding code. |
...
citation, and neither was defaulted to S4 out of habit -- both are pure
compositions of quantities this project's own code already computes and
already independently validated:

- **7.4 (Toeplitz-matrix cache)** caches the exact return value of
  `fourier_factorization.toeplitz_matrix`/`toeplitz_matrix_component`,
  already transcribed from `S4/S4/pattern/pattern.c` (Phase 2's citation)
  and `S4/S4/fmm/fmm_closed.cpp` (Phase 6's citation) -- caching introduces
  no new formula, only a key/lookup layer around an already-cited one. See
  `design.md`'s "Layer/Toeplitz Caching Design" and `decisions.md` ADR-016.
- **7.6 (layer-wise absorption)** is a z-Poynting-flux-divergence
  combination of `smatrix.interior_amplitudes` (Category 9 target 9.3,
  independently derived, `decisions.md` ADR-015), `fields.propagate_amplitudes`
  (Category 9 target 9.2), and `fields.z_poynting_flux` (Phase 1,
  transcribed from `S4/S4/rcwa.cpp::GetZPoyntingFlux`) -- deliberately
  *not* a new volumetric `Im(eps)*|E|^2` integral formula, per
  `decisions.md` ADR-017's reasoning. No vendored repo needed to be
  consulted for this target because it doesn't introduce any new physics,
  only a new combination of already-cited/already-validated pieces.
