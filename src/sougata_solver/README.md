...
| [`config.py`](config.py) | Category 15 targets 15.2-15.4: `simulation_from_dict`/`_json_string`/`_json_file` — minimal JSON simulation-configuration schema (stdlib `json` only), reusing `geometry_io.py`'s existing material/pattern sub-schemas; validation is structural (only ever constructs objects, never calls `.solve()`) |
| [`cli.py`](cli.py) | Category 15 targets 15.5/15.6: `main`, the `sougata-solver run <config.json>` command-line entry point — exit codes `0`/`1`/`2` (success/solver failure/invalid config), output via `output_paths.py`'s existing dated-folder convention |
| [`export.py`](export.py) | Category 15 target 15.7: `export_sweep_npz`/`load_sweep_npz` — serializes a `sweep.SweepResult` to a NumPy `.npz` archive, metadata JSON-encoded into a string array (no `allow_pickle=True` needed on load) |
| [`plotting.py`](plotting.py) | Category 16 targets 16.1-16.7: `plot_unit_cell`/`plot_layer_stack`/`plot_rt_spectrum`/`plot_harmonic_convergence`/`plot_diffraction_orders`/`plot_field_intensity`/`plot_field_phase`/`plot_poynting_vector` — every function takes plain arrays/already-computed result objects (never a `Simulation`) and returns `(fig, ax)`; never calls `.solve()` |
