"""Phase 5 example: linearly-tapered square Si pillar, staircase-discretized.

Stack (top to bottom):
    air                            (incidence, semi-infinite)
    Si pillar, side length tapering (num_slices staircase layers,
      top_side -> bottom_side       2D-periodic, see staircase.py)
    air                            (exit, semi-infinite)

Complements `tapered_via.py` (Circle) and `tapered_trench.py` (Slab): this
is the `Rectangle`-shape case of the same Phase 5 staircase generator
(`staircase_rectangle_layers`), here with equal x/y halfwidths so the cross-
section is a tapered square pillar rather than an independently-tapered
rectangle. FDTD-style tcd/bcd/depth/spacing naming, see conversation citing
phases.md Phase 5/staircase.py.

Sweeps `num_slices` at a fixed wavelength/angle and prints R/T so the
convergence-vs-N trend (`staircase.py`'s docstring: this phase's
correctness evidence, no external oracle exists for tapered geometry) is
visible directly, mirroring `tests/test_staircase.py`'s tapered-rectangle
convergence checks. Per ADR-010, this script only prints/saves raw numbers
-- no plotting here.

Run with:  python structures/via/tapered_pillar.py
"""

from __future__ import annotations

import math

import numpy as np

from sougata_solver.excitation import PlaneWaveExcitation
from sougata_solver.geometry import Lattice
from sougata_solver.materials import Material
from sougata_solver.output_paths import run_output_dir, write_run_metadata
from sougata_solver.simulation import Simulation
from sougata_solver.staircase import staircase_rectangle_layers

# ============================================================================
# EDIT: tapered pillar geometry and materials (FDTD-style tcd/bcd/depth/
# spacing naming; tcd/bcd here are square pillar side lengths, top and
# bottom face)
# ============================================================================
TCD = 0.42e-6                 # top critical dimension: pillar side length at incidence-side face (meters)
BCD = 0.18e-6                 # bottom critical dimension: pillar side length at transmission-side face (meters)
SPACING = 0.28e-6             # edge-to-edge gap to the neighboring pillar at the top face (meters)
PERIOD = TCD + SPACING         # lattice period (meters)
THICKNESS = 0.46e-6             # pillar height (meters)
N_PILLAR = 3.48                  # pillar index (e.g. Si)
N_BG = 1.0                        # background index (air)

# ============================================================================
# EDIT: incident light -- angle, polarization, order truncation
# ============================================================================
WAVELENGTH = 1.0e-6
INCIDENT_ANGLE_DEG = 0.0
NUM_ORDERS = 5
S_AMPLITUDE = 1.0
P_AMPLITUDE = 0.0

# ============================================================================
# EDIT: slice-count sweep
# ============================================================================
SLICE_COUNTS = [1, 2, 4, 8, 16, 32]

OUTPUT_CSV = "output_tapered_pillar_convergence.csv"


def main() -> None:
    air = Material("air", N_BG**2)
    pillar = Material("pillar", N_PILLAR**2)
    lattice = Lattice(a=(PERIOD, 0.0), b=(0.0, PERIOD))
    excitation = PlaneWaveExcitation(
        wavelength=WAVELENGTH,
        theta=math.radians(INCIDENT_ANGLE_DEG),
        phi=0.0,
        s_amplitude=S_AMPLITUDE,
        p_amplitude=P_AMPLITUDE,
    )

    reflectance = np.zeros(len(SLICE_COUNTS))
    transmittance = np.zeros(len(SLICE_COUNTS))

    print(f"{'num_slices':>10}  {'R':>10}  {'T':>10}  {'R+T':>10}")
    for i, num_slices in enumerate(SLICE_COUNTS):
        layers = staircase_rectangle_layers(
            center=(PERIOD / 2, PERIOD / 2),
            top_halfwidth=(0.5 * TCD, 0.5 * TCD),
            bottom_halfwidth=(0.5 * BCD, 0.5 * BCD),
            thickness=THICKNESS,
            num_slices=num_slices,
            shape_material=pillar,
            background_material=air,
            name_prefix="pillar",
        )
        sim = Simulation(lattice, layers, num_orders=NUM_ORDERS, incidence=air, transmission=air)
        result = sim.solve(excitation)
        reflectance[i] = result.reflectance()
        transmittance[i] = result.transmittance()
        rt = reflectance[i] + transmittance[i]
        print(f"{num_slices:10d}  {reflectance[i]:10.6f}  {transmittance[i]:10.6f}  {rt:10.6f}")

    out = run_output_dir("tapered_pillar")
    csv_path = out / OUTPUT_CSV
    table = np.column_stack([SLICE_COUNTS, reflectance, transmittance])
    np.savetxt(csv_path, table, delimiter=",", header="num_slices,R,T", comments="")
    write_run_metadata(
        out,
        __file__,
        period_m=PERIOD,
        tcd_m=TCD,
        bcd_m=BCD,
        spacing_m=SPACING,
        thickness_m=THICKNESS,
        n_pillar=N_PILLAR,
        n_bg=N_BG,
        wavelength_m=WAVELENGTH,
        incident_angle_deg=INCIDENT_ANGLE_DEG,
        num_orders=NUM_ORDERS,
        s_amplitude=S_AMPLITUDE,
        p_amplitude=P_AMPLITUDE,
        slice_counts=SLICE_COUNTS,
    )
    print(f"\nSaved {len(SLICE_COUNTS)} rows to {csv_path}")
    print(f"Run metadata: {out / 'run_metadata.txt'}")


if __name__ == "__main__":
    main()
