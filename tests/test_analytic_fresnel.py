...
scratch, not derived from EMpy or sougata_solver)."""
...
def test_lossless_multilayer_matches_fresnel_oracle(structure_name, theta_deg, polarization):
    ...
    r_sougata_solver, t_sougata_solver, r_oracle, t_oracle = _run(
        n_incidence=1.0,
    ...
    assert (r_sougata_solver + t_sougata_solver) == pytest.approx(1.0, abs=1e-8)
...
def test_absorbing_layer_matches_fresnel_oracle(theta_deg, polarization):
    ...
    assert r_sougata_solver == pytest.approx(r_oracle, abs=1e-8)
    assert t_sougata_solver == pytest.approx(t_oracle, abs=1e-8)
    assert r_sougata_solver + t_sougata_solver < 1.0  # lossy: some power absorbed
...
def test_thick_absorptive_layer_is_numerically_stable():
    ...
    assert np.isfinite(r_sougata_solver)
    assert np.isfinite(t_sougata_solver)
    assert r_sougata_solver == pytest.approx(r_oracle, abs=1e-8)
    assert t_sougata_solver == pytest.approx(t_oracle, abs=1e-6)
    assert t_sougata_solver < 1e-10  # essentially fully absorbed
