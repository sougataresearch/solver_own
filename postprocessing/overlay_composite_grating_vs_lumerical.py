"""Overlay `multistack_composite_grating.py`'s R/T output against a Lumerical
RCWA `grating_power` export (see `export_grating_power.lsf`, this
conversation).

Handles a known Lumerical-script quirk: its `write()` appends rather than
overwrites, so re-running the export script without deleting the previous
output leaves multiple header+data blocks concatenated in one file --
`_load_lumerical_grating_power` below finds every "lambda*" header line and
uses only the last (most recent) block.

Plots R and T on two panels, solver vs. Lumerical, and prints a max/RMS
difference -- Lumerical's wavelength grid (near-uniform in *frequency*,
per its printed values) does not exactly coincide with the solver's
(uniform in *wavelength*), so the Lumerical curve is linearly interpolated
onto the solver's grid before differencing; this is a numerical comparison
aid, not a claim the two grids are identical.

Run with:  python postprocessing/overlay_composite_grating_vs_lumerical.py
"""
...
# use. "Rs"/"Ts" (s/TE) matches this solver's default S_AMPLITUDE=1.0/
# P_AMPLITUDE=0.0 in multistack_composite_grating.py -- switch to "Rp"/"Tp"
# if the Lumerical excitation was actually p/TM.
# ============================================================================
SOLVER_CSV = (
    r"C:\Users\sougata.bhunia\Desktop\Solver_own\sougata_solver\outputs\2026_08_18"
    r"\13_53_18_multistack_composite_grating\output_multistack_composite_grating_RT.csv"
)
LUMERICAL_TXT = (
    r"C:\Users\sougata.bhunia\Desktop\Solver_own\sougata_solver\OUTPUT_RCWA\Thin_Film"
    r"\Complex_Multi\composite_grating_lumerical_RCWA.txt"
)
...
PLOT_FILENAME = "composite_grating_vs_lumerical.png"
