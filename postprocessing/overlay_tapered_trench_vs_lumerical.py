"""Overlay `tapered_trench.py`'s R/T output against a Lumerical RCWA
`grating_power` export (`export_trench_grating_power.lsf`, this
conversation).

Same known Lumerical-script quirk as
`overlay_composite_grating_vs_lumerical.py`: its `write()` appends rather
than overwrites, so re-running the export script without deleting the
previous output leaves multiple header+data blocks concatenated in one
file -- `_load_lumerical_grating_power` below finds every "lambda*" header
line and uses only the last (most recent) block.
...
Run with:  python postprocessing/overlay_tapered_trench_vs_lumerical.py
"""
...
# use. "Rs"/"Ts" (s/TE) matches tapered_trench.py's S_AMPLITUDE=1.0/
# P_AMPLITUDE=0.0 -- switch to "Rp"/"Tp" if the Lumerical excitation was
# actually p/TM.
# ============================================================================
SOLVER_CSV = (
    r"C:\Users\sougata.bhunia\Desktop\Solver_own\sougata_solver\outputs\2026_08_19"
    r"\10_26_00_tapered_trench\output_trench_RT.csv"
)
LUMERICAL_TXT = (
    r"C:\Users\sougata.bhunia\Desktop\Solver_own\sougata_solver\OUTPUT_RCWA\Trench"
    r"\trench_lumerical_RCWA.txt"
)
...
PLOT_FILENAME = "tapered_trench_vs_lumerical.png"
